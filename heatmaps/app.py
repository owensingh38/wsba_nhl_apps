    ...
    def plot_skater():
        ...
        if 'strength_state' not in query.keys():
            if 'skater' not in query.keys():
                query = {'skater':['8473419'],'season':['20182019'],'team':['BOS'],'strength_state':['5v5']}
            else:
                query.update({'strength_state':['5v5']})
        ...
        df = df.loc[(df['season'].astype(str).isin(query['season']))].replace({np.nan: None})
